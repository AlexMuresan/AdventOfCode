...
for move in instructions:
    ...
    box_list[dest - 1].insert(0, box_list[source - 1][:nr_boxes])
    box_list[dest - 1] = list(
        itertools.chain.from_iterable(np.asarray(b).ravel() for b in box_list[dest - 1]))
    del box_list[source - 1][:nr_boxes]
